...
function searchRealisatorByName(){
  console.log("je suis la")
  if (selectedActorId) {
    window.location.href = `/realisator.html?id=${selectedActorId}`;  // Redirection vers la page de l'acteur
} else {
    alert("Veuillez sélectionner un réalisateur dans la liste des suggestions.");
}
}


function autocompleteRealisator() {
  var input = document.getElementById('actor-name').value;
  if (input.length > 2) {  // Suggérer à partir de 3 caractères saisis
      fetch(`http://localhost:8080/realisators/auto-complete/${input}`)
